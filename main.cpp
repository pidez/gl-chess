...
int main(int argc, char** argv) {
	...
	sphere->loadMesh("models/models_definitivi/componenti/scacchieraobj", aiProcess_FlipUVs);
	rook->loadMesh("models/models_definitivi/rook/rook.obj", aiProcess_FlipUVs);
	rook->setModelTransform(glm::translate(glm::scale(glm::mat4(1.0), glm::vec3(3, 3, 3)), glm::vec3(0.2, -0.2, 0)));
	sphere->setModelTransform(glm::scale(glm::mat4(1.0), glm::vec3(8, 8, 8)));
	...
	rookProgram->setLightPosition(glm::vec3(3, 3, 0));
	...
	sphereProgram->setLightPosition(glm::vec3(3, 3, 1));
...
void keyboardFunction(unsigned char key, int x, int y) {
	...
	if(key == 'd') {
		scene->rotateScene(glm::vec3(0, 0, 1), -1.0f);
	}

	if(key == 's') {
		scene->rotateScene(glm::vec3(1, 0, 0), 1.0f);
	}
